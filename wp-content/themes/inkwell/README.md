...
ACF Pro is required for the component blocks. All blocks include their field definitions, including the original hero exported with its existing keys. The 12 design-library components are `hero-split`, `logo-strip`, `icon-grid`, `process-steps`, `team-grid`, `featured-case-study`, `latest-posts`, `form-content`, `anchor-nav`, `content-columns`, `full-width-media` and `quote-feature`. Each `acf-fields.json` can be imported through ACF Tools, while the theme also registers it automatically when the component is present. Client content and media remain in WordPress.

The extended library adds `sticky-side-content`, `image-hotspots`, `before-after-slider`, `metric-story`, `feature-showcase`, `service-selector`, `resource-hub`, `results-breakdown`, `video-testimonial`, `sticky-anchor-sidebar`, `steps-with-media` and `pricing-matrix`. These use the shared Section Settings field structure for background, vertical spacing and container width. Pricing supports up to three packages; Video Testimonial accepts YouTube or Vimeo URLs and loads the embed only after activation.
